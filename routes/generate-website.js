...
router.post('/generate-website', async (req, res) => {
    ...
    // Now you can use resumeText for whatever processing you need
    ...
    const websiteTitleCompletion = await openai.chat.completions.create({
        messages: [{
            role: "system", content: `You are an AI assistant tasked with generating a brief and versatile HTML section for a user's personal profile website, 
                ...
                Based on the user's resume: ${resumeText}... create a concise HTML header section that captures their unique professional identity and expertise. follow the structure and format of the titles in the template very closely by using the exact same tags and attributes.
                `
        }],
    ...
    const websiteSubTitleCompletion = await openai.chat.completions.create({
        messages: [{
            role: "system", content: `here is the resume of the applicant:  ${resumeText}... Create a professional and engaging one-liner for a personal profile website's subtitle. The one-liner should be suited for a
                 job application and reflect the user's enthusiasm.  the one-liner should be related to the applicant's background information and personal experience. in the one-liner, include the company and job they are applying to.  ...example of a one liner could be: "Passionate community development coordinator with a proven track record of networking with public officials and advocating for social justice. Excited to bring my expertise in fundraising and strategic planning to contribute to Helping Hands Foundation."...
                
               
               

                The one-liner should be formatted as an HTML paragraph (<p>) with id="text04" and class="style1". Example format: <p id="text04" class="style1">[Generated content here]</p>.
                remember to cross reference the html you are generating and the user profile to make sure that the information is accurate.Only generate the html. Do not include any text/comments/recommendations/warnings before or after that is not html.
               `
    ...
    const websiteExperience1Completion = await openai.chat.completions.create({
        messages: [{
            role: "system", content: ` User Profile Overview (websiteExperiences variable):
                ${websiteExperiences}...
                - applicant's resume: ${resumeText}...
                Generate a detailed description for the first professional experience listed in the 'websiteExperiences' variable. The content should match the format of the provided HTML template and be as engaging and informative as the example. Here are the specifics:
    ...
    const websiteExperience2Completion = await openai.chat.completions.create({
        messages: [{
            role: "system", content: ` User Profile Overview (websiteExperiences variable):
                ${websiteExperiences}...
                - here is the resume of the applicant: ${resumeText}...Generate a detailed description for the second professional experience listed in the 'websiteExperiences' variable. The content should match the format of the provided HTML template and be as engaging and informative as the example. Here are the specifics:
    ...
    const websiteExperience3Completion = await openai.chat.completions.create({
        messages: [{
            role: "system", content: ` User Profile Overview (websiteExperiences variable):
                ${websiteExperiences}...
                - applicant's resume: ${resumeText}...
                Generate a detailed description for the third professional experience listed in the 'websiteExperiences' variable. The content should match the format of the provided HTML template and be as engaging and informative as the example. Here are the specifics:
